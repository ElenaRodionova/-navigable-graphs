...
from my_graph import Graph, _calculate_recall
from my_HNSW import my_HNSW
...
def main():
    ...
    #Create my_graph
    #my_graph = Graph(k = args.K, M = args.M, dist_func = l2_distance, data = train_data)

    # Calculate recall
    #recall, avg_cal = _calculate_recall(my_graph, test_data, groundtruth_data, k=args.k, ef=args.ef, m=args.m)
    #print(f"Average recall: {recall}, avg calc: {avg_cal}")
    
    # Create my_HNSW
    print('my HNSW')
    hnsw = my_HNSW( distance_func=l2_distance, m=args.M, m0=args.M0, ef=10, ef_construction=30,  neighborhood_construction = heuristic)
    # Add data to HNSW
    ...
    print(f"Average recall my HNSW: {recall}, avg calc: {avg_cal}")
