...
def heuristic(candidates, curr, k, distance_func, data):
    ...
    for c, curr_dist in candidates[:k]: 
...
class my_HNSW:
    ...
    def add(self, elem, ef=None):
        ...
        level = int(-log2(random.random()) * self._level_mult)
        level = 1
        idx = len(data)
        data.append(elem)
        ...
        if point is not None:
            ...
            for layer in reversed(graphs[:level]):
                ...
                candidates = self.beam_search(graph=layer, q=elem, k=int(level_m/3), eps=[point], ef=self._ef_construction)
